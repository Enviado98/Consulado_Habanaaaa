...
function updateAdminUI(isAdmin) {
    ...
    if (isAdmin) {
        ...
        DOMElements.toggleAdminBtn.textContent = "🛡️ MODO EDICIÓN"; 
...
function toggleAdminMode() {
    if (!admin) {
        ...
        if (!confirm("✅️ ¿Guardar cambios o salir sin guardar?")) return;
        updateAdminUI(false);
        loadData(); 
        loadStatusData(); 
    }
...
function renderStatusPanel(status, isAdminMode) {
    ...
    if (isAdminMode) {
        ...
        // NOTA: Quitamos estilos inline para que el CSS funcione.
        DOMElements.statusDataContainer.innerHTML = `
            <div class="status-item">
                <span class="label">Deficit (MW):</span>
                <input type="text" id="editDeficit" value="${status.deficit_mw || ''}">
            </div>
            <div class="status-item">
                <span class="label">USD (Auto):</span>
                <input type="text" value="${status.dollar_cup}" disabled>
            </div>
            <div class="status-item">
                <span class="label">EUR (Auto):</span>
                <input type="text" value="${status.euro_cup}" disabled>
            </div>
        `;
    } else {
        // MODO PÚBLICO
